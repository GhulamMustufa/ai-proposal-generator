...
export class SubmitterService {
  ...
  async submitApplication(applicationId: string, userId: string, jobId: string) {
    ...
    try {
      ...
      const [app] = await this.db.select().from(applications).where(eq(applications.id, applicationId)).limit(1);
      ...
      try {
        ...
        await page.fill('input[name="name"]', fullName).catch(() => addLog('Name field not found'));
        await page.fill('input[name="email"]', contact.email).catch(() => addLog('Email field not found'));
        
        if (contact.phone) {
          await page.fill('input[name="phone"]', contact.phone).catch(() => addLog('Phone field not found'));
        }
        
        if (contact.linkedin) {
          await page.fill('input[name="urls[LinkedIn]"]', contact.linkedin).catch(() => addLog('LinkedIn field not found'));
        }
        ...
        const submitBtn = page.locator('button.postings-btn.template-btn-submit');
        await submitBtn.click().catch(() => addLog('Submit button not clicked'));
        
        // Wait for potential redirect or success message
        addLog('Waiting for success confirmation network idle...');
        await page.waitForNavigation({ waitUntil: 'networkidle', timeout: 5000 }).catch(() => addLog('No navigation detected after submit (this may be normal depending on the ATS).'));
