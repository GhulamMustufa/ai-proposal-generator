...
export class AtsIngestionService {
  ...
  async scrapeSmartRecruiters(): Promise<string[]> {
    ...
    const companies = await this.db.select().from(targetCompanies).where(eq(targetCompanies.atsProvider, 'smartrecruiters'));
    this.logger.log(`Found ${companies.length} SmartRecruiters companies to scrape.`);
    ...
    for (const company of companies) {
      ...
      try {
        ...
        for (const job of data?.content || []) {
          ...
          let description = '';
          try {
            const detailRes = await fetch(`https://api.smartrecruiters.com/v1/companies/${company.atsBoardToken}/postings/${job.id}`);
            if (detailRes.ok) {
              const detailData = await detailRes.json();
              description = cleanDescription(detailData.jobAd?.sections?.jobDescription?.text || '');
            }
          } catch(e) {}
          
          if (!description) description = `Job title: ${job.name}. Apply at ${job.ref}`;
          ...
          const inserted = await this.db.insert(jobs).values({
            ...
            company: company.name,
            description: description.slice(0, 7000),
            url: job.ref || `https://jobs.smartrecruiters.com/${company.atsBoardToken}/${job.id}`,
          }).onConflictDoNothing().returning({ id: jobs.id });
          
          if (inserted.length > 0) newJobIds.push(inserted[0].id);
        }
      } catch (error) {
        this.logger.error(`Failed to scrape SmartRecruiters board ${company.atsBoardToken}`);
      }
    }
    
    this.logger.log(`Ingested ${newJobIds.length} total jobs from all SmartRecruiters boards.`);
    return newJobIds;
  }

  async scrapeWorkable(): Promise<string[]> {
    this.logger.log('Starting Universal Workable ATS scraping...');
    const companies = await this.db.select().from(targetCompanies).where(eq(targetCompanies.atsProvider, 'workable'));
    ...
    for (const company of companies) {
      ...
      try {
        ...
        this.logger.error(`Failed to scrape Workable board ${company.atsBoardToken}`);
      }
    }
    
    this.logger.log(`Ingested ${newJobIds.length} total jobs from all Workable boards.`);
    return newJobIds;
  }

  async scrapeBreezy(): Promise<string[]> {
    this.logger.log('Starting Universal Breezy HR ATS scraping...');
    const companies = await this.db.select().from(targetCompanies).where(eq(targetCompanies.atsProvider, 'breezy'));
      ...
      try {
        ...
        for (const job of jobsList || []) {
          ...
          const inserted = await this.db.insert(jobs).values({
            ...
            url: job.url || `https://${company.atsBoardToken}.breezy.hr/p/${job._id}`,
          }).onConflictDoNothing().returning({ id: jobs.id });
          
          if (inserted.length > 0) newJobIds.push(inserted[0].id);
        }
      } catch (error) {
        this.logger.error(`Failed to scrape Breezy board ${company.atsBoardToken}`);
      }
    }
    
    this.logger.log(`Ingested ${newJobIds.length} total jobs from all Breezy boards.`);
    return newJobIds;
  }

  async scrapeAshby(): Promise<string[]> {
    this.logger.log('Starting Universal Ashby HQ ATS scraping...');
    const companies = await this.db.select().from(targetCompanies).where(eq(targetCompanies.atsProvider, 'ashby'));
      ...
      try {
        const response = await fetch(url, {
          ...
          body: JSON.stringify({
            ...
            variables: { organizationHostedJobsPageName: company.atsBoardToken },
        ...
        for (const job of postings) {
          ...
          const inserted = await this.db.insert(jobs).values({
            ...
            url: job.jobPageUrl || `https://jobs.ashbyhq.com/${company.atsBoardToken}/${job.id}`,
          }).onConflictDoNothing().returning({ id: jobs.id });
          
          if (inserted.length > 0) newJobIds.push(inserted[0].id);
        }
      } catch (error) {
        this.logger.error(`Failed to scrape Ashby board ${company.atsBoardToken}`);
      }
    }
    
    this.logger.log(`Ingested ${newJobIds.length} total jobs from all Ashby boards.`);
    return newJobIds;
  }
}
